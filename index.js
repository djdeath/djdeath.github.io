...
;
  ...
  ;
    ...
    Polymer('noflo-main', {
      ...
      localruntime: function (runtimeDescription) {
        if (runtimeDescription.protocol === 'iframe') {
          return;
        }
        var runtime = new this.registry.Runtime(runtimeDescription, {
      ...
      createdproject: function (project) {
        window.location.hash = '#project/' + encodeURIComponent(project.id) + '/' + project.main;
      },
  ...
  ;
    ...
    Polymer('noflo-runtime', {
      ...
      wasConnected: false,
      graphsChanged: function () {
          console.log('runtime: graphs changed! prev=' +
                      (this.currentGraph ? this.currentGraph.name : 'none'));
      ...
      clearRuntime: function () {
        for (var i = 0; i < this.runtimes.length; i++) {
          ...
          console.log('runtime: clearRuntime!');
        this.runtime = null;
        this._currentGraphChanged();
      },
      _currentGraphChanged: function () {
        this.wasConnected = false;
        if (this.runtime) {
          console.log('runtime: currentGraphChanged!');
        ...
        this.currentGraph.on('changeProperties', function (newProps, oldProps) {
              console.log('runtime: changed graph properties');
      ...
      runtimeStatusChanged: function(status) {
         console.log('runtime: runtimeStatusChanged');
          console.log(status);
        ...
        this.runtime.sendNetwork('list', {});
      },
      runtimeChanged: function () {
          console.log('runtime: runtimeChanged');
        ...
        if (!this.runtime) {
          ...
          console.log('runtime: setting main!');
          console.log(this.currentGraph);
        this.runtime.setMain(this.currentGraph);
        this.runtime.on('status', this.runtimeStatusChanged.bind(this));
        ...
        this.runtime.on('network', function (message) {
            console.log('runtime: network message:');
            console.log(message);
        ...
        this.runtimeStatusChanged(this.runtime.status);
      },
      ...
      notify: function (title, message) {
        ...
        if (this.lastNotification && this.lastNotification.title === title && this.lastNotification.body === message) {
          console.log(this.lastNotification, this.lastNotification.title, title, this.lastNotification.title === title, this.lastNotification.body, message, this.lastNotification.body === message);
